...
export const update = async (req, res) => {
  try {
    const updatedData = await ecbUserRegistration.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true })
    if (!updatedData) {
      return res.status(404).json({ message: 'Data not found' })
    }
...
export const deleteRecord = async (req, res) => {
  try {
    const deletedData = await ecbUserRegistration.findByIdAndDelete(req.params.id)
    if (!deletedData) {
      return res.status(404).json({ message: 'Data not found' })
    }
