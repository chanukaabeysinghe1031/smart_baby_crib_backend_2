...
function broadcastWS(message) {
  console.log(`Broadcasting message to WebSocket clients: ${JSON.stringify(message)}`);
...
app.post('/api/distance/reset', (req, res) => {
  ...
  res.status(200).send({ success: true, message: 'Distance reset successfully.' });
});
...
app.post('/api/distance/resume', (req, res) => {
  ...
  res.status(200).send({ success: true, message: 'Distance tracking resumed.' });
});
...
app.post('/api/steer', (req, res) => {
  ...
  if (typeof steering !== 'number' || steering < -100 || steering > 100) {
    ...
    return res.status(400).send({ success: false, message: 'Invalid steering value' });
  }
...
app.post('/api/remote', (req, res) => {
  ...
  if (!['phone', 'ring'].includes(remote)) {
    ...
    return res.status(400).send({ success: false, message: 'Invalid remote control option' });
  }
